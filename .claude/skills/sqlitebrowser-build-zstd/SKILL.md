...
Locate the repository root, then read `.agents/skills/sqlitebrowser-build-zstd/SKILL.md` completely and follow it. Resolve every repository-relative path from that root. The canonical instructions and `third_party/zstd/build.cmd` are the single sources of truth; do not copy or reinterpret the build workflow in this compatibility file.
