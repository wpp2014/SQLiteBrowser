...
~~~cmd
git clone --branch master --recurse-submodules https://github.com/wpp2014/SQLiteBrowser.git
cd SQLiteBrowser
git submodule update --init --recursive
~~~

After v4.0.0 is published, replace `--branch master` with
`--branch v4.0.0` to reproduce the released source exactly.
...
The Release preset always sets `BUILD_STABLE_VERSION=ON`, so the executable
uses the stable `4.0.0.0` Windows file/product version rather than a dated
nightly version. Both v4 presets disable the upstream application update check;
the HTTPS endpoint used by the explicit runtime smoke test remains independently
configurable and enabled.
...
a release package. Assemble the strict runtime-only directories with:
...
## 8. Build the Release x64 portable self-extracting EXE

The NSIS package is an extraction-only portable wrapper around the same strict
70-file Release runtime used by the MSI. It does not install a product,
...
This is install-free packaging, not a dedicated portable-settings mode.
Application settings can still use the normal registry or AppData paths.

## 9. Build the Release x64 MSI with WiX
...
its OSMF EULA. Local builds do not accept the EULA automatically. After
...
made and documented that licensing decision. The repository's GitHub-hosted
...
output\x64-shared-release\package\artifacts\
  ...
  DB.Browser.for.SQLCipher-4.0.0-win-x64.msi.sha256
...
Interactive installation presents a `Shortcut Options` page after install
directory selection. The all-users Start Menu shortcut inside a
`DB Browser for SQLCipher` product folder and the all-users desktop shortcut are
both enabled by default, and either can be disabled independently. Silent
installation keeps both defaults; deployment automation can pass
`CREATE_DESKTOP_SHORTCUT=0` and/or `CREATE_START_MENU_SHORTCUT=0` to `msiexec`
to suppress them. Both shortcuts target the installed application and use the
executable's embedded icon. Repair, major upgrade, and uninstall validation must
cover every selection combination; uninstall must also remove the now-empty
Start Menu product folder when that shortcut was selected.

The publication runtime explicitly excludes `vc_redist.x64.exe`, so SFX and MSI
do not distribute or execute the redistributable installer. No app-local VC143
runtime is currently added in its place. A clean machine must therefore already
have a compatible Visual C++ x64 Runtime until the app-local runtime policy is
implemented and verified. Use Microsoft's current x64 package from
<https://aka.ms/vc14/vc_redist.x64.exe>.

The v4.0.0 SFX and MSI are not Authenticode-signed. Windows can display an
unknown-publisher or SmartScreen warning. Verify each downloaded package with
its adjacent `.sha256` file before running it.

The v4 presets set `SQLITEBROWSER_ENABLE_UPDATE_CHECK=OFF`. This prevents the
SQLCipher-only fork build from querying the upstream DB Browser for SQLite
release endpoint and directing users to a different binary product.
...
git tag -a v4.0.0-rc.1 -m "DB Browser for SQLCipher 4.0.0 RC1"
git push origin v4.0.0-rc.1
~~~

After the resulting draft has been downloaded and manually verified, create the
final `v4.0.0` tag from the exact green `master` commit. The final draft must
contain exactly these four files:
...
Review and publish the draft manually in GitHub. The workflow refuses to
replace an existing Release. Configure repository tag rules so only authorised
maintainers can create the `v4.*` release tags. Automatic WinGet publication is
disabled; its workflow can only be started manually.
...
For the current portable NSIS SFX, WiX MSI, and legacy-install migration
decisions, see the
